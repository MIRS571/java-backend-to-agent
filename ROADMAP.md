...
| 离线质量门槛 | 完成 | Ruff、45 个 pytest 测试和 MkDocs 严格构建通过 |
...
- 当前阶段：第 1～7 章内容完成，不代表学习者已经掌握。
- 已交付：第 1～6 章完成 Agent、LangChain、Tool Calling 与企业 RAG 主线；第 7 章完成 TypedDict State、局部更新、Reducer、普通/条件 Edge、START/END、编译与调用、MessagesState、ToolNode 和工具循环。
...
- 已验证：`uv sync --locked`、`uv run ruff check .`、45 个 pytest 测试、`uv run mkdocs build --strict`，以及第 1～7 章的离线示例运行命令。
- 已知局限：第 7 章用确定性函数模拟模型决策，State 只存在于单次调用；不验证真实模型、Checkpointer、Runtime、Interrupt、跨调用记忆、异步流式服务或生产工具授权。
- 下一入口：第 8 章“Checkpointer、Memory、Runtime、Interrupt 与 Resume”。
