...
| 离线质量门槛 | 完成 | Ruff、8 个 pytest 测试和 MkDocs 严格构建通过 |
...
| 2 | Java 开发者所需的 Python、uv、异步与 FastAPI | 待开始 |
...
- 当前阶段：第 1 章内容完成，不代表学习者已经掌握。
- 已交付：第 1 章正文、显式上下文离线示例和 3 个章节测试；示例可观察“不带历史”与“带历史”的结果差异。
- 验证环境：Python 3.12，`pytest 9.1.1`、`ruff 0.16.5`、`MkDocs 1.6.1`、`mkdocs-material 9.7.7`。
- 已验证：`uv sync --locked`、`uv run ruff check .`、8 个 pytest 测试、`uv run mkdocs build --strict`、章节示例运行命令。
- 已知局限：本章使用确定性 Fake Model 解释输入边界，不测试真实模型能力、厂商 SDK、Tool Calling 或 Agent 循环。
- 下一入口：第 2 章“Java 开发者所需的 Python、uv、异步与 FastAPI”。
